...
for i, age_group in enumerate(age_groups):
    ...
    axs[row, col].set_ylabel('Tote')
    axs[row, col].plot(timespan_data.dates(), timespan_data.moved_to_n_days_past(21).values("new_infections_weekly_sum"), label="Wöchentliche Neuinfektionen", color='green')
    axs[row, col].plot(timespan_data.dates(), timespan_data.values("new_deaths_weekly"), label="Wöchentliche Summe", color='blue')
    axs[row, col].plot(timespan_data.dates(), timespan_data.values("new_deaths"), color='lightgray', linestyle='dotted', label='Täglich')
